...
function GalaxLib:CreateWindow(opts)
    ...
    function WIN:_render()
        ...
        for i,tab in ipairs(self._tabs) do
            ...
            if open then poolAdd(pool,"tab_ul_"..i,"Square",{Position=tpos+Vector2.new(0,tabH-2),Size=Vector2.new(tw,2),Filled=true,Color=T.Accent,Transparency=1-self._fadeAlpha,Visible=true,ZIndex=4})
            ...
            if Input.click and over(tpos,tsz) then
                ...
                self._fadeAlpha=0
                task.spawn(function()
                    local t0=tick(); local DUR=0.25
                    repeat task.wait(); self._fadeAlpha=math.min((tick()-t0)/DUR,1) until self._fadeAlpha>=1
                end)
            end
            tabX=tabX+tw+5
        end
        ...
        if self._fadeAlpha < 1 then
            ...
            for id,obj in pairs(pool.d) do
                if id:sub(1,1)=="s" then obj.Transparency=inv end
            end
        end
    end
    ...
    task.spawn(function()
        WIN:_buildSettings()
        setrobloxinput(false)  -- block game input immediately on open
        -- fade in the first tab on startup
        task.spawn(function()
            local t0=tick(); local DUR=0.25
            repeat task.wait(); WIN._fadeAlpha=math.min((tick()-t0)/DUR,1) until WIN._fadeAlpha>=1
        end)
