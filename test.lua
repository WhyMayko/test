--[[
    ...
    Sec:AddToggle("Enable", false, function(v) end, 0x46)  -- com keybind opcional
    ...
    -- Dropdown e MultiDropdown tem :Refresh(newOptions, newDefault)
...
function GalaxLib:CreateWindow(opts)
    ...
    -- [FIX] bloqueia input na abertura se o roblox estiver ativo
    ...
    function WIN:AddTab(name)
        ...
        function TAB:AddSection(sname)
            ...
            -- [CHANGE] keybind é o 4º parâmetro opcional
            -- AddToggle("label", default, cb)          -- sem keybind
            -- AddToggle("label", default, cb, 0x46)    -- com keybind
            function SEC:AddToggle(label,default,cb,keybind)
                local item={
                    type="toggle", label=label, value=default or false,
                    cb=cb or function()end,
                    keybind=keybind or nil,   -- nil = sem keybind
                    kb_listening=false, kb_wasM1=false,
                }
                reg(item); item.cb(item.value)
                return{
                    Get=function()return item.value end,
                    Set=function(_,v)item.value=v;item.cb(v)end,
                }
            end
            ...
            function SEC:AddDropdown(label,options,default,opts2,cb)
                ...
                return{
                    Get=function()return item.value end,
                    Set=function(_,v)item.value=v;item.cb(v)end,
                    -- [NEW] Refresh: atualiza options e reseta seleção
                    Refresh=function(_,newOptions,newDefault)
                        item.options = newOptions or {}
                        item.scroll  = 0
                        item.value   = newDefault or (newOptions and newOptions[1]) or ""
                        item.cb(item.value)
                    end,
                }
            end
            ...
            function SEC:AddMultiDropdown(label,options,default,opts2,cb)
                ...
                return{
                    ...
                    Refresh=function(_,newOptions,newDefault)
                        ...
                        item.selected = {}
                        item.scroll   = 0
                        if newDefault then for _,v in ipairs(newDefault) do item.selected[v]=true end end
                        item.cb(getList())
                    end,
                }
            end
    ...
    function WIN:_renderWidget(item,pool,wid,wx,wy,innerW,FONT)
        ...
        elseif item.type=="toggle" then
            ...
            -- [CHANGE] keybind badge opcional no lado direito do checkbox
            local kbW=0
            if item.keybind ~= nil then
                local kbStr = item.kb_listening and "[ ... ]" or ("["..keyName(item.keybind).."]")
                local kbBadgeW = textW(kbStr,11)+8
                local kbPos = Vector2.new(wx+innerW-kbBadgeW, wy)
                local kbSz  = Vector2.new(kbBadgeW, 14)
                poolAdd(pool,wid.."_kbbg","Square",{Position=kbPos,Size=kbSz,Filled=true, Color=item.kb_listening and T.AccentDark or T.Surface1,Visible=true,ZIndex=6})
                poolAdd(pool,wid.."_kbb", "Square",{Position=kbPos,Size=kbSz,Filled=false,Color=item.kb_listening and T.Accent or T.Border0,Thickness=1,Visible=true,ZIndex=7})
                poolAdd(pool,wid.."_kbt", "Text",  {Position=kbPos+Vector2.new(3,1),Text=kbStr,Size=11,Font=FONT,Color=item.kb_listening and T.Accent or T.SubText,Outline=false,Visible=true,ZIndex=7})
                -- detecta click no badge
                local m1=Input.held
                if m1 and not item.kb_wasM1 and over(kbPos,kbSz) then
                    item.kb_listening=true
                end
                item.kb_wasM1=m1
                -- escuta tecla
                if item.kb_listening then
                    for kc=1,255 do
                        if kc~=0x01 and kc~=0x02 and Input:keyClick(kc) then
                            if kc~=0x1B then item.keybind=kc end
                            item.kb_listening=false; break
                        end
                    end
                end
                -- dispara toggle pela keybind
                if item.keybind and item.keybind~=0 and not item.kb_listening then
                    if Input:keyClick(item.keybind) then
                        item.value=not item.value; item.cb(item.value)
                    end
                end
                kbW = kbBadgeW+4
            end
            -- checkbox fica antes do badge (ou na direita se sem badge)
            local bpos=Vector2.new(wx+innerW-14-kbW, wy)
            ...
            -- click na área do label/checkbox (excluindo badge)
            if Input.click and over(Vector2.new(wx,wy),Vector2.new(innerW-kbW-2,16)) then
                item.value=not item.value; item.cb(item.value)
            end
            wY=22
        ...
        elseif item.type=="button" then
            ...
            -- [FIX] texto centralizado verticalmente: bsz.Y/2 - fontSize/2 = 22/2 - 13/2 ≈ 4.5
            poolAdd(pool,wid.."_btnt","Text",  {Position=bpos+Vector2.new(innerW/2, math.floor(bsz.Y/2-6)),Text=item.label,Size=13,Font=FONT,Color=T.Text,Center=true,Outline=false,Visible=true,ZIndex=7})
        ...
        elseif item.type=="settings_kill" then
            ...
            -- [FIX] centralizado também no kill button
            poolAdd(pool,wid.."_btnt","Text",  {Position=bpos+Vector2.new(innerW/2, math.floor(bsz.Y/2-6)),Text=item.label,Size=13,Font=FONT,Color=T.Red,Center=true,Outline=false,Visible=true,ZIndex=7})
    ...
    task.spawn(function()
        ...
        while WIN._running do
            ...
            -- [FIX] fora do roblox e menu aberto: desbloqueia input
            if not rbx then
                if WIN._open then setrobloxinput(true) end
                continue
            end
            -- dentro do roblox: processa input normalmente
            Input:update()
            ...
            -- [FIX] garante bloqueio enquanto menu aberto no roblox
            if WIN._open then setrobloxinput(false) end
            WIN:_render()
        end
